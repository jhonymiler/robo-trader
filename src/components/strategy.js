...
export function executeStrategy(candleData, operations, updateOperations, initialCapital = 500, notify) {
    const emaLength = 9;
    const atrLength = 14;
    ...
    // Saldo inicial e cálculo da fração de BTC que pode ser comprada
    let capital = initialCapital;  // Saldo disponível
    const btcQty = capital * 0.9 / closes[lastCandleIndex];   // Fração de BTC que pode ser comprada
    let stop = lows[lastCandleIndex] - trailingStopInitial;
    ...
    if (operations.length > 0) {
        ...
        if (!lastOperation.sellPrice) {
            // Atualizando o trailing stop dinamicamente
            let updatedTrailingStop = Math.max(lastOperation.stop, closes[lastCandleIndex] - atr * trailingStopInitial);
            lastOperation.stop = updatedTrailingStop;

            // Verificar se o preço atual caiu abaixo do trailing stop
            if (closes[lastCandleIndex] < updatedTrailingStop) {
                // Executar venda
                ...
                updateOperations(lastOperation);
